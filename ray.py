...
class Ray:
    def __init__(self, angle, player, map, controls) -> None:
    ...
    def cast(self):
        # bunch of geometry here to find ray length
        ...
        # DISTANCE CALCULATION AND SHORTEST DISTANCE -> WALL HIT COORDS
        ...
        if not self.controls.fish_eye_effect:
            self.distance *= math.cos(self.player.rotationAngle - self.rayAngle)
        
        if self.controls.shading_effect:
            self.color *= (self.controls.shading_distance / self.distance)
            if self.color > 255:
                self.color = 255
            elif self.color < 0:
                self.color = 0
